...
ostream& operator<<(ostream &os, node* head) {
	print(head);
	return os; //cout object //Cascading of operators(can cout multiple objects -> linked list)
}

//Overloading the right shift(>>) operator
istream& operator>>(istream &is, node*&head) { //Modifying the original linked list so put &
	head = take_input();
	return is;
}
